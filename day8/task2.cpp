...
//    rules.open("D:/Python/Advent-of-code-2020-python/day7/input.txt", std::ios::in);
    ...
    for (int i=0 ; i < len ; i++){
        ...
        int iter = 0;
        int status = 0;
        std::vector<std::string> copy_instructions = instructions;

        if( copy_instructions[i].substr(0, 3) == "jmp" ){
            copy_instructions[i].replace(0,3, "nop");
            status ++;
        }
        if (( copy_instructions[i].substr(0, 3) == "nop" ) && ( status == 0 )){
            copy_instructions[i].replace(0,3, "jmp");
        }

        while (copy_instructions[current_index] != "used"){
            iter ++;
            if ( iter > 10000 ){
                break;
            }
            temp_instruction = copy_instructions[current_index];
            copy_instructions[current_index] = "used";
            ...
            if (copy_instructions.back() == "used") {
